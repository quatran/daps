...
ifneq "$(strip $(NOT_VALIDATE_TABLES))" "1"
  FAULTY_TABLES := $(shell XML_CATALOG_FILES=/etc/xml/catalog $(LIBEXEC_DIR)/validate-tables.py $(PROFILED_MAIN) 2>&1 | sed -r -e 's,^/([^/: ]+/)*,,' -e 's,.http://docbook.org/ns/docbook.,,' | sed -rn '/^- / !p' | tr "\n" "@")
endif
